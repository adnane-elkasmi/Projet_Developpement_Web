...
#----------------------------------------------------------#
# Graphe de Probabilité de ruine en fonction du Capital c  #
#-------------- -------------------------------------------#


def grapheProba_ruine_Q(x,T,λ,α,β,Nmc):
    c=np.linspace(0.,50.,50)
    y=[Proba_ruine_Q(i,x,T,λ,α,β,Nmc) for i in c]
    plot(c,y)
    xlabel("Capital c")
    ylabel("Probabilité de la ruine")
    title("Graphe de probabilité de ruine en fonction du capital")
    show()
...
def grapheProba_ruine_Q_T(c,x,λ,α,β,Nmc):
    t=np.linspace(0.1,20.,20)
    y=[Proba_ruine_Q(c,x,i,λ,α,β,Nmc) for i in t]
    plot(t,y)
    xlabel("Horizon T")
    ylabel("Probabilité de la ruine")
    title("Graphe de probabilité de ruine en fonction du horizon T")
    show()
